...
Three stages, current split across two files:
...
2. **AI** (`prompt_contract_layer.py`) — takes one fetch-layer record and
   ...
   + a plain-language narrative. This is the only stage that's genuinely
...
ships only the golden-set descriptions and the eval output (status +
...
contract (e.g. no rule yet for "this commit is done but the feature it's
part of still isn't usable"), and at least two are cases where the agent's
