...
def main():
    ...
    for r in rows:
        ...
        if corr:
            ...
            corrected.append((gf, corr["original_status"], corr["matched_phrases"]))
        ok = golden == gen
        match += ok
        print(f"{gf:<10} {commit:<16} {golden:<14} {gen:<14} {'ok' if ok else 'MISS':<4} {corr_str}")
    print("-" * 78)
    print(f"{match}/{len(rows)} match on status")
    print(f"validator fired on {len(corrected)}/{len(rows)} entries"
          + (":" if corrected else ""))
    for gf, original_status, phrases in corrected:
        print(f"  {gf}: {original_status} -> Pending via {phrases}")
    return 0
